...
\
	for (uint i = uint(0); i < ACTIVE_SPOTS(cluster); i++) { \
		...
		float atten = spot_attenuation(idx, POSITION); \
		float shadow = spot_shadow(idx, POSITION); \
...
\
	for (uint i = uint(0); i < ACTIVE_DIRECTIONAL(cluster); i++) { \
		...
		float atten = directional_attenuation(idx, POSITION); \
		vec3 dir = POSITION - DIRECTIONAL_LIGHT(idx).direction.xyz; \
